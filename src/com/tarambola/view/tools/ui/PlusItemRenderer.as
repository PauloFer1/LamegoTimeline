package com.tarambola.view.tools.ui
{
	...
	public class PlusItemRenderer extends FeathersControl implements IListItemRenderer
	{
		...
		private var _data:PlusItem;
		private var _text:TextField;
		private var _line:Line;
		private var _border:Image;
		
		public function PlusItemRenderer()
		{
			super();
			this.width=180;
			this.height = 56;
		...
		override protected function initialize():void
		{
			...
			var font2:BitmapFont = Fonts.getInstance().getFont(Constants.FONT_LIST_NAME2);
			font2.smoothing = TextureSmoothing.TRILINEAR;
			this._text = new TextField(98,42, "", font2.name, 14, 0x4D060B);
			...
			{
				this.width = 180;
				font = Fonts.getInstance().getFont(Constants.FONT_LIST_NAME2);
				this._text.fontName = font.name;
			...
			else
			{
				...
				this._text.fontName = font.name;
				this._text.color = 0x000000;
				this._text.fontSize = 16;
				this._text.alpha = 0.7;
				this._text.x = 80;
				
			}
			
			this._line = new Line(0,0,1,0x404041);
			this._line.lineTo(this.width, 0);
			this._line.alpha = 0.5;
			this.addChild(this._line);
			
			this._border = new Image(Arq.getInstance().model.asset.getTexture("image_border_"+Constants.getInstance().actUI));
			this._border.y=6;
		...
		override protected function draw():void
		{
			...
			if(dataInvalid)
			{
				...
				if(this._data)
				{
					...
					this._text.text = this._data.title;
			...
			if(this.image.isLoaded)
			{
				...
				this.image.width = this._border.width-6;
				
				var scale:Number = 0;
				if(scale==0)
					scale=(this.image.image.height/(this._border.height-6))/100 / (this.image.image.width/(this._border.width-6));
				this.image.height = this._border.height-6;
				this.imageContainer.x=3;
				this.image.image.setTexCoords(0,new Point(0,0));
				this.image.image.setTexCoords(1,new Point(1,0));
				this.image.image.setTexCoords(2,new Point(0,1-scale));
				this.image.image.setTexCoords(3,new Point(1,1-scale));				
			}
			//this.image.y = this._border.height/2 - this.image.height/2 - 2;
		//	this.image.image. .clipRect = new Rectangle(0,0,this._border.width-6, 50);
			//trace(this.image.clipRect.width);
		}
		//**************** PRIVATE METHODS ***************//
		...
		private function touchHandler(event:TouchEvent):void
		{
			...
			if(this.touchPointID >= 0)
			{
				...
				if(touch.phase == TouchPhase.ENDED)
				{
					...
					if(isInBounds)
					{
						NavEvents.getInstance().dispatchCustomEvent("GALLERY.SELECTED", "plusrenderer", {id:this._data.id});
